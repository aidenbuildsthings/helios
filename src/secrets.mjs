...
async function windowsCrypt(value, decrypt = false) {
  const script = decrypt
    ? "$s=[Console]::In.ReadToEnd()|ConvertTo-SecureString;$p=[System.Net.NetworkCredential]::new('', $s);[Console]::Out.Write($p.Password)"
    : "$s=ConvertTo-SecureString ([Console]::In.ReadToEnd()) -AsPlainText -Force;[Console]::Out.Write((ConvertFrom-SecureString $s))";
  return runWithInput("powershell.exe", ["-NoLogo", "-NoProfile", "-NonInteractive", "-Command", script], value);
}
