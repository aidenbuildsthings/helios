  ...
  return [
    {
      ...
      run: ({ path: file }) => readFile(resolveWorkspacePath(workspace, file), "utf8"),
    },
    {
      ...
      run: async ({ path: directory = "." }) => {
        const root = resolveWorkspacePath(workspace, directory);
    ...
    },
    {
      ...
      run: async ({ command, args = [] }) => {
        ...
        const result = await execFileAsync(command, args, { cwd: workspace, timeout: 120_000, maxBuffer: 2_000_000 });
